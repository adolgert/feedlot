...
bool AdjacentPens(int i, int j, const PenContactGraph& g) {
  auto adj=adjacent_vertices(i, g);
  for ( ; adj.first!=adj.second; ++adj.first) {
    if (*adj.first==j) {
      return true;
    }
  }
...
BuildSystem(int64_t individual_cnt, int block_cnt, int row_cnt)
{
  ...
  enum : int64_t { none, infect, infectious, recover };
  ...
  for (int64_t d_idx=0; d_idx<individual_cnt; ++d_idx) {
    int pen_d=(d_idx-1)/pen_cnt;
    for (int64_t s_idx=0; s_idx<individual_cnt; ++s_idx) {
      int pen_s=(s_idx-1)/pen_cnt;
      if (s_idx!=d_idx) {
        if (pen_d==pen_s) {
          bg.AddTransition({d_idx, s_idx, infect},
            {Edge{{d_idx, location, s}, -1}, Edge{{s_idx, location, i}, -1}, 
                Edge{{d_idx, location, e}, 2}},
